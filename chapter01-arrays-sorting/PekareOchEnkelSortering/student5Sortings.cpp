...
void bubbleSortP(float *pBegin, float *pEnd){
    ...
    for(float *p =pEnd-1;p>pBegin;p-=1)
    {
        for(float *pStart = pBegin; pStart < p; pStart+=1){
            if(*pStart>*(pStart+1)){
...
/*
    ...
    for(int i=0; i<size-1; i+=1){
        float x = pBegin[i+1];
        int curval = i;
        for( ; curval >= 0 && pBegin[curval] > x; curval-=1)
         {
            pBegin[curval+1] = pBegin[curval];
        }
        pBegin[curval+1] = x;
    }
*/
...
void studentsTest5(){
    ...
    cout << "Dina egna tester måste komma här!\n";
